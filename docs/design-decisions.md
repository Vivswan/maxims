...
- **`--cooldown` and `--cap` live in state under `config`.** Machine-wide numbers belong with intent, not on a hook command line that would have to repeat them.
...
- **DeepSeek Harness is tier 1 through its Claude Code hook bridge, caveats recorded.** The bridge reads its path once at start and has no per-project discovery; its config points at a maxims-owned file, never into `.claude`.
...
- **The rule file is a real file, never a symlink, on every harness.** Claude Code skips a symlinked rule file pointing outside the working directory, and a rule that silently never loads is the failure the tool exists to prevent.
