...
- **One canonical home, `~/.agents/maxims/`, with links everywhere else.** The only arrangement in which a stale body cannot exist; `--copy` is the escape hatch. Details on the [files page](files.md#the-canonical-home).
...
- **Two scopes only, project and user; `-o` is an escape hatch, not a third scope.** Every harness has a project target and most have a user target, so two scopes cover the matrix; `-o` exists for the one rule file no harness owns. The [install page](install.md#where-it-lands) owns the shapes.
- **User defaults live in `config.json` beside state, never in state.** A default is a preference about future commands, not intent about installed sources; keeping it out of `state.json` means the intent-only rule needs no exception. This replaces the earlier `state.config` shape. The [defaults section](files.md#user-defaults-in-configjson) owns the keys.
- **A project commits `.agents/maxims.lock` and `install` replays it.** State is per machine, so a fresh clone would otherwise start with no rules; the lock is what a teammate's `install` reads. Sorted keys and no timestamps keep its diff to what changed. The [share page](share.md) owns the file.
...
- **Exceeding the [rule cap](keep-fresh.md#the-cap-and-the-cooldown) refuses the whole source.** Truncating would drop the last rule silently and make the result depend on sort order.
...
- **`--rename <upstream>=<local>` resolves a collision without a prompt.** A scripted `add` that meets exit 6 has no way forward otherwise; the flag records the same entry the prompt would. The [collision section](install.md#name-collisions-and-renames) owns it.
- **Hidden characters in a memory refuse the install unless `--allow-hidden`.** A character the reader cannot see can carry an instruction past the plan, and the plan is the review gate. The [hidden-character rule](write-memories.md#hidden-characters-are-refused) owns what counts and the exit code.
- **`metadata.internal: true` hides a memory unless the installer opts in.** A source repo can hold rules for its own maintainers without a second repo; the marker is in the file, so it travels with it. The [contract](write-memories.md#the-contract) owns the field and the opt-in variable.
...
- **Fetching is anonymous by default; `--auth` opts a source in and is recorded as `intent.auth`.** Reading a `gh` token nobody asked to use turns a public-repo install into an authenticated request, and a token read is a side effect the plan never showed. The [fetch section](keep-fresh.md#how-a-source-is-fetched) owns the order of attempts and the environment variables.
- **Any git URL is a source, stored verbatim, cloned as typed, with no tarball fallback.** Host-specific resolution would need a resolver per forge; a clone works on all of them. The [canonical home](files.md#the-canonical-home) owns its store path.
...
- **`doctor` checks what a harness would load; `--expect` and `--json` make it a CI assertion.** `list` reports state; `doctor` reads the harness's own loading rules against disk, which is the only way to catch a rule file at the wrong path. The [doctor section](check.md#doctor-what-each-harness-loads) owns it.
- **`init [name]` ships.** Scaffolding one contract-valid file is cheaper than explaining the contract; the [init section](write-memories.md#scaffold-a-file-with-init) owns what it writes.
...
- **Non-interactively, `--yes` is implied for `add` and `sync` only; `remove` without an explicit `-y` aborts with exit 1.** Both implied verbs are idempotent and reversible; a deletion is not. `remove --all` passes because `--all` spells `-y` out. The [non-interactive table](install.md#non-interactive-behavior) owns the matrix.
