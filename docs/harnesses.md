...
- **`project target` and `global target`** are the file written for a project install and for `-g`; `none` means that scope is skipped. A block cell that lists files is written into the first of them the harness reads, the first that exists or, where the cell says so, the first that is not empty; the named file is created only when none qualifies.
...
- **Codex instruction files.** Codex prefers `AGENTS.override.md` over `AGENTS.md`, but its two loaders treat a blank override differently. Its home loader skips it and reads `~/.codex/AGENTS.md`, so the block goes there. Its project loader takes it and reads nothing else from that directory, so the block goes into the blank override. Codex stops reading instruction files past 32 KiB combined by default.
