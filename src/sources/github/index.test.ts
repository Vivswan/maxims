...
// network, a live source asked to fetch, or a fetched tree read from the wrong folder would each
// cost every session start or install the wrong memories.
import { describe, expect, test } from "bun:test";
import { withTempDir } from "../../../tests/shared/temp_dir.ts";
import { exited, ghScript, httpResponse, scriptedRunner } from "./fixtures/runner.ts";
...
describe("resolveRef", () => {
  test("a full sha, in any case, resolves without touching a rung", async () => {
    ...
    const resolver = createGithubResolver({ runner, warn: () => {} });
  ...
  test("a pin replaces the stored ref for that resolution only", async () => {
    const runner = scriptedRunner({
      fetch: (url) => httpResponse(200, url.endsWith("/v2") ? SHA : "x"),
    });
    const resolver = createGithubResolver({ runner, warn: () => {} });
...
describe("fetch", () => {
  test("resolves once, extracts into tempDir/tree, and reads memories relative to the repo root", async () => {
    await withTempDir(async (tempDir) => {
      ...
      const resolver = createGithubResolver({ runner, warn: (m) => warnings.push(m) });
  ...
  test("a pre-resolved sha in from.ref skips resolution, and fullDepth reads the whole tree", async () => {
    await withTempDir(async (tempDir) => {
      const runner = scriptedRunner({ fetch: () => httpResponse(200, cleanTarball()) });
      const resolver = createGithubResolver({ runner, warn: () => {} });
      const result = await resolver.fetch(
        { ...FROM, ref: SHA },
        { memoryPath: "memories", fullDepth: true, tempDir },
      );
      ...
      expect(runner.calls).toEqual([
        "exec gh auth status --hostname github.com",
        `git clone https://github.com/Example-User/rules.git ${SHA}`,
