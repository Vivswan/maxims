...
import type { RenameMap, Select } from "../state/schema.ts";
import { ExitCode } from "../util/exit-codes.ts";
import { DEFAULT_RULE_CAP } from "./budget.ts";
import {
  buildNameIndex,
  type Candidate,
  type IndexedSource,
...
describe("resolveSourceCandidates", () => {
  ...
  test("26 survivors are refused whole at the default cap; 25 render", () => {
    ...
    expect(resolve(memories)).toEqual({
      ...
      count: 26,
      cap: 25,
      hint: "narrow the source with --memory <name>..., or raise config.ruleCap (currently 25)",
    });
