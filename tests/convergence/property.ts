// The two knobs every property run reads: MAXIMS_PROPERTY_ITERATIONS is the run count (8 by
// default; a deep run sets hundreds), MAXIMS_PROPERTY_SEED replays one failing run. A failure
// names the seed to set, so a red run elsewhere is reproduced here with one variable.
...
// run eight iterations while the caller believes it ran two hundred, or a fresh seed while the
...
export function propertyKnobs(env: NodeJS.ProcessEnv = process.env): PropertyKnobs {
  const numRuns = knob(env, "MAXIMS_PROPERTY_ITERATIONS", /^[1-9]\d*$/, [
    1,
    Number.MAX_SAFE_INTEGER,
  ]);
  const seed = knob(env, "MAXIMS_PROPERTY_SEED", /^-?\d+$/, [-(2 ** 31), 2 ** 31 - 1]);
  return { numRuns: numRuns ?? DEFAULT_ITERATIONS, seed };
}
